...
class ChannelWiseLayerNorm(nn.LayerNorm):
    ...
    How does LN work about T x C or C x T?
    """
...
class GlobalChannelLayerNorm(nn.Module):
    ...
    def __init__(self, dim, eps=1e-05, elementwise_affine=True):
        ...
        if elementwise_affine:
            self.beta = nn.Parameter(t.zeros(dim, 1))
            self.gamma = nn.Parameter(t.ones(dim, 1))
        else:
    ...
    def forward(self, x):
        ...
        if x.dim() != 3:
            raise RuntimeError("{} accept 3D tensor as input".format(self.__name__))
        # N x 1 x 1
        mean = t.mean(x, (1, 2), keepdim=True)
        var = t.mean((x - mean)**2, (1, 2), keepdim=True)
        # N x T x C
        if self.elementwise_affine:
            x = self.gamma * (x - mean) / t.sqrt(var + self.eps) + self.beta
        else:
...
class Conv1D(nn.Conv1d):
    ...
    def forward(self, x, squeeze=False):
        ...
        x = super().forward(x if x.dim() == 3 else t.unsqueeze(x, 1))
        if squeeze:
            x = t.squeeze(x)
        return x
...
class ConvTasNet(nn.Module):
    def __init__(self,
        ...
        # self.conv1x1_2 = t.nn.ModuleList(
    ...
    def forward(self, x):
        ...
        y = self.repeats(y)
        # n x 2N x T
        e = t.chunk(self.mask(y), self.num_spks, 1)
        # n x N x T
        if self.non_linear_type == "softmax":
            m = self.non_linear(t.stack(e, dim=0), dim=0)
        else:
            m = self.non_linear(t.stack(e, dim=0))
        # spks x [n x N x T]
